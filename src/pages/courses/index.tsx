...
const Courses = () => {
  ...
  const addBranch = (id: number) => {
    if (!selectedBranchIds.includes(id)) setSelectedBranchIds((prev) => [...prev, id]);
  };
  const removeBranch = (id: number) => setSelectedBranchIds((prev) => prev.filter((x) => x !== id));

  const addLevel = (id: number) => {
    if (!selectedLevelIds.includes(id)) setSelectedLevelIds((prev) => [...prev, id]);
  };
  const removeLevel = (id: number) => setSelectedLevelIds((prev) => prev.filter((x) => x !== id));
          ...
          <div className="expenses-subcategory" style={{ minWidth: 480 }}>
            <h1>
              {editingItem
                ...
                : t('courses.addTitle', 'Kurs q‘shish')}
            </h1>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  >
                    ...
                    {selectedBranchIds.length === 0 && (
                      <span style={{ color: '#888', fontSize: 14 }}>{t('courses.choose')}</span>
                    )}
                  </div>
          ...
          <thead>
            <tr>
              <th>
                <input
                  type="checkbox"
                  checked={(courses?.length ?? 0) > 0 && selected.length === (courses?.length ?? 0)}
