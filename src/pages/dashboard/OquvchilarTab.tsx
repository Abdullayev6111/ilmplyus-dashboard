...
function ScoreBadge({ pct }: { pct: number }) {
  let bg = "#dcfce7";
  let color = "#16a34a";
  if (pct < 80) { bg = "#fef9c3"; color = "#ca8a04"; }
  if (pct < 60) { bg = "#fee2e2"; color = "#dc2626"; }
  return (
...
export default function OquvchilarTab() {
  ...
  const levelData = useMemo(() => {
    ...
    (groupsData?.list ?? []).forEach((g) => {
      const name = g.level?.name_uz || g.level?.name || "Boshqa";
          ...
          {levelData.length > 0 ? (
            <ResponsiveContainer width="100%" height={280}>
              ...
              >
                ...
                />
                <Tooltip
                  formatter={(v: number | undefined) => [`${v ?? 0} ta`, t("dashboard.oquvchi.count")]}
                  cursor={{ fill: "rgba(59,130,246,0.06)" }}
                />
                <Legend wrapperStyle={{ fontSize: 12, fontFamily: "noto-r" }} />
                <Bar dataKey="value" name={t("dashboard.oquvchi.count")} fill="#3b82f6" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
          ...
          <ResponsiveContainer width="100%" height={240}>
            ...
            >
              ...
              <YAxis
                ...
                tickFormatter={(v) => `${v}`}
              />
              <Tooltip formatter={(v: number | undefined) => [`${v ?? 0}%`, t("dashboard.oquvchi.avgBall")]} />
              <Legend wrapperStyle={{ fontSize: 12, fontFamily: "noto-r" }} />
              <Area
                type="monotone"
                dataKey="ball"
      ...
      <div className="dash-mid-row">
        <div className="dash-card">
          <h3 className="dash-card__title">
            <i className="fa-solid fa-calendar-check" style={{ marginRight: 8, color: "#6366f1" }} />
          ...
          <ResponsiveContainer width="100%" height={220}>
            ...
            >
              ...
              <YAxis
                ...
                tickFormatter={(v) => `${v}`}
              />
              <Tooltip formatter={(v: number | undefined) => [`${v ?? 0}%`, t("dashboard.oquvchi.attendancePct")]} />
              <Legend wrapperStyle={{ fontSize: 12, fontFamily: "noto-r" }} />
              <Bar dataKey="pct" name={t("dashboard.oquvchi.attendancePct")} fill="#3b82f6" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
