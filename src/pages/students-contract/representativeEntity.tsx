...
const RepresentativeCard = ({
  ...
  index: number;
  onChange: (index: number, field: keyof RepresentativeFormData, value: string | string[]) => void;
  onRemove: (index: number) => void;
  showRemove: boolean;
}) => {
  const { t } = useTranslation();
          ...
          <div className="sc-form-col" style={{ flex: '0 0 32%' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              {pIdx === representative.phones.length - 1 ? (
                <div
                  onClick={() => onChange(index, 'phones', [...representative.phones, '+998'])}
                  style={{
                    background: '#003366',
                    color: '#fff',
                    height: '42px',
                ...
                <div
                  onClick={() => {
                    const newPhones = representative.phones.filter((_, i) => i !== pIdx);
                    onChange(index, 'phones', newPhones);
                  }}
                  style={{
                    background: '#fee2e2',
  ...
  useEffect(() => {
    if (contractToEdit && id) {
      ...
      if (contract.representatives && contract.representatives.length > 0) {
        setRepresentatives(
          contract.representatives.map((r: any) => ({
            ...
            birth_date: formatDateForInput(r.birth_date),
            passport_given_date: formatDateForInput(r.passport_given_date),
            passport_expiry_date: formatDateForInput(r.passport_expiry_date),
